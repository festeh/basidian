...
export const api = {
  ...
  async getNodeByPath(path: string): Promise<FsNode> {
    const response = await fetch(
      `${BASE_URL}/fs/node?path=${encodeURIComponent(path)}`,
    );
    return handleResponse<FsNode>(response);
  },
  ...
  async searchFiles(query: string): Promise<FsNode[]> {
    const response = await fetch(
      `${BASE_URL}/fs/search?q=${encodeURIComponent(query)}`,
    );
    return handleResponse<FsNode[]>(response);
  },
  ...
  async getVersion(nodeId: string, versionId: string): Promise<FileVersion> {
    const response = await fetch(
      `${BASE_URL}/fs/node/${nodeId}/versions/${versionId}`,
    );
  ...
  async restoreVersion(
    ...
    const response = await fetch(
      `${BASE_URL}/fs/node/${nodeId}/restore/${versionId}`,
      {
        method: "POST",
      },
    );
